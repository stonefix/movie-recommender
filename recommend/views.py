    ...
    if request.method == "POST":
        ...
        if user is not None:
            if user.is_active:
                ...
                return render(request, 'recommend/login.html', {'error_message': 'Ошибка, нету такого аккаунта'})
        else:
            return render(request, 'recommend/login.html', {'error_message': 'Ошибка логина'})
